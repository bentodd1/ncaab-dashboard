-- ============================================
-- Import All CSV Data - Skip Duplicates
-- ============================================
USE SportsAnalytics;
GO
...
WHERE NOT EXISTS (
    ...
    WHERE g.SportID = 1
    AND g.SeasonID = @Season2122
    AND g.GameDate = CONVERT(DATE, s.date, 101)
    AND g.HomeTeam = s.home
    AND g.RoadTeam = s.road
);

PRINT 'Inserted ' + CAST(@@ROWCOUNT AS VARCHAR) + ' new games for 2021-22';
...
WHERE NOT EXISTS (
    SELECT 1 FROM dbo.Games g
    WHERE g.SportID = 1
    AND g.SeasonID = @Season2223
    AND g.GameDate = CONVERT(DATE, s.date, 101)
    AND g.HomeTeam = s.home
    AND g.RoadTeam = s.road
);

PRINT 'Inserted ' + CAST(@@ROWCOUNT AS VARCHAR) + ' new games for 2022-23';
TRUNCATE TABLE dbo.StagingGames;
GO
...
WHERE NOT EXISTS (
    SELECT 1 FROM dbo.Games g
    WHERE g.SportID = 1
    AND g.SeasonID = @Season2324
    AND g.GameDate = CONVERT(DATE, s.date, 101)
    AND g.HomeTeam = s.home
    AND g.RoadTeam = s.road
);

PRINT 'Inserted ' + CAST(@@ROWCOUNT AS VARCHAR) + ' new games for 2023-24';
GO
