...
# Find sqlcmd.exe
Write-Host "Locating sqlcmd.exe..." -ForegroundColor Yellow
$sqlcmdPaths = @(
...
if (-not $sqlcmd) {
    Write-Host "ERROR: Could not find sqlcmd.exe" -ForegroundColor Red
    Write-Host "Please install SQL Server Command Line Utilities from:" -ForegroundColor Yellow
    Write-Host "https://learn.microsoft.com/en-us/sql/tools/sqlcmd/sqlcmd-utility" -ForegroundColor Cyan
    exit
}

Write-Host "  Found: $sqlcmd" -ForegroundColor Green

$serverInstance = "MSI\SQLEXPRESS"
$scriptPath = "C:\Users\happy\Documents\Projects\ncaab-prediction-tracker\sql"

# Step 1: Create Database
Write-Host "`n[1/6] Creating database..." -ForegroundColor Yellow
